...
class AgentRouter:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.mode = str(self.config.get("mode", os.getenv("AGENT_MODE", "ppo_only"))).lower()
        ...
        self.hybrid_threshold = int(self.config.get("hybrid_threshold", 10))
        self.turn_counter = 0
    ...
    def predict_action(self, observation: Any, state: Dict[str, Any]) -> int:
        ...
        if self.default_agent == "hf_llm_agent":
            ...
            if self.mode == "full_llm" or self.full_llm:
                ...
                use_hf = False
        elif self.default_agent == "hybrid":
            use_hf = self._should_use_hf(network_risk)
        else:
            use_hf = False
    ...
    def get_active_agent_name(self) -> str:
        ...
        if self.last_used_agent == "random":
    ...
    def switch_agent(self, agent_name: str) -> bool:
        ...
        if agent_name == "ppo_agent":
            if not self.is_agent_available("ppo_agent"):
                return False
            self.default_agent = "ppo_agent"
            self.mode = "ppo_only"
            self.full_llm = False
        elif agent_name == "hf_llm_agent":
            if not self.is_agent_available("hf_llm_agent"):
                return False
            self.default_agent = "hf_llm_agent"
            self.full_llm = True
        elif agent_name == "hybrid":
            self.default_agent = "hybrid"
            self.mode = "hybrid"
            self.full_llm = False
        elif agent_name == "random":
            self.default_agent = "random"
